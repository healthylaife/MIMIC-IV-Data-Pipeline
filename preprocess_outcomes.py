...
def load_data(dataFile, labelFile, typeFile, dist=False, exclude=[]):
    ...
    data_lbl_v1.columns = [
        ...
        "mort_label",
        "LOS",
    ]  # ,"vent_label","time_to_intub","Readmission_label","plos_label"]
    data_lbl = pd.merge(
        data_dat["Pt_id"].drop_duplicates(), data_lbl_v1, how="inner"
    ).drop_duplicates()
    print(
        "loaded labels for: ",
        data_lbl_v1["Pt_id"].nunique(),
        " after primary cleaning ",
        data_lbl["Pt_id"].nunique(),
    )
    print(
        "Mortality Case counts: ",
        data_lbl[data_lbl["mort_label"] == 1]["Pt_id"].nunique(),
    )
...
def pickle_data(data_dat, data_lbl, types, reverse=True):
    ...
    for Pt, group in data_dat.groupby("Pt_id"):
        ...
        for Time, subgroup in group.sort_values(
            ["Time"], ascending=not reverse
        ).groupby(
            "Time", sort=False
        ):  ### ascending=True normal order ascending=False reveresed order
            data_i_c.append(
                np.array(subgroup["ICD"]).tolist()
            )  # get ICD codes for each admission separately
            data_dt_c.append(
                dt.strptime(Time, "%Y-%m-%d")
            )  # concat dischargetime of each admission
        if len(data_i_c) > 0:
...
def split_data(fset, pt_list, pts_file_pre, outFile):
    ...
    if pts_file_pre == "NA":
        ...
        test_indices = np.intersect1d(
            pt_list, pt_test, assume_unique=True, return_indices=True
        )[1]
        valid_indices = np.intersect1d(
            pt_list, pt_valid, assume_unique=True, return_indices=True
        )[1]
        train_indices = np.intersect1d(
            pt_list, pt_train, assume_unique=True, return_indices=True
        )[1]
...
def dump_split_process_data(
    ...
    data_dat, data_lbl, types = load_data(
        dataFile, labelFile, typeFile, dist=dist, exclude=exclude
    )
    types, pt_list, label_list, newVisit_list, dur_list = pickle_data(
        data_dat, data_lbl, types, reverse=reverse
    )
...
if __name__ == "__main__":
    ...
    dump_split_process_data(
        dataFile, labelFile, typeFile, outFile, pts_file_pre, dist=False, exclude=[]
    )
