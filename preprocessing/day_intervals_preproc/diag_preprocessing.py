...
def get_patients(module_path: str, use_ICU: bool) -> pd.DataFrame:
    """Reads in the patients module; optionally only returns subject_ids
    that exist in icustays"""

    pts = pd.read_csv(
        module_path + "/core/patients.csv.gz", compression="gzip", header=0
    )
    ...
    if use_ICU:
        icu_ids = pd.read_csv(
            module_path + "/icu/icustays.csv.gz", compression="gzip", header=0
        )["subject_id"].unique()

        pts = pts.merge(
            pd.Series(icu_ids, name="subject_id"),
            left_on="subject_id",
            right_on="subject_id",
            how="inner",
        )

    return pts.loc[pts.yob >= 18]
...
def preproc_icd_module(
    ...
    pos_ids = pd.DataFrame(
        diag.loc[diag.root.str.contains(ICD10_code)].subject_id.unique(),
        columns=["subject_id"],
    )
    pos_ids["label"] = 1

    cohort = pts.merge(pos_ids, left_on="subject_id", right_on="subject_id", how="left")
    cohort.fillna(0, inplace=True)

    return cohort, errors


def extract_diag_cohort(
    use_ICU: str,
    label: str,
    module_path="./mimic-iv-1.0",
    ...
    cohort, errors = preproc_icd_module(
        module_path, label, icd_map_path, use_ICU == "ICU"
    )
    cohort_output = f"cohort_{use_ICU}_{label}"
    summary_output = f"summary_{use_ICU}_{label}"
    cohort.to_csv(
        f"./data/day_intervals/cohort/{cohort_output}.csv.gz",
        index=False,
        compression="gzip",
    )
    ...
    summary = "\n".join(
        [
            f"{label} FOR {use_ICU} COHORT DATA",
    ...
    # save errors associated with ICD9 -> ICD10 mapping
    with open(f"./data/day_intervals/cohort/ICD_convert_errors.txt", "w") as f:
        f.write(errors)

    print("[ ERRORS SUCCESSFULLY SAVED ]")
...
if __name__ == "__main__":
    ...
    print("Please select a ICD10 root category to label by:")
    print("Examples:\nCKD\t\tN18\nCAD\t\tI25\nCOPD\t\tJ44\nHeart Failure\tI50\n")
    ICD10_code = input("Enter code:    ").strip().upper()
    use_ICU = input("Use ICU Data? (ICU/Non_ICU)\n").strip()
    extract_diag_cohort(use_ICU, ICD10_code)
